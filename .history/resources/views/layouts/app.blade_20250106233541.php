            ...
            @if (!request()->is('login') || request()->is('register'))
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        @if (Auth::user())
                        {{ Auth::user()->name }}
                        <img class="rounded-circle" src="images/users/{{json_decode(Auth::user()->image)}}" width="60" height="60">
                        @endif
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        <li><a class="btn btn-dark dropdown-item" href="{{ route("users.edit",Auth::user()->id)}}" type="submit">Edit Profile</a></li>
                            <form class="pt-2" action="{{route("logout")}}" method="POST">
                                @method("POST")
                                @csrf
                                <li><button class="btn dropdown-item" type="submit">Logout</button></li>
                            </form>
                        @endif
                    </ul>
                </li>
