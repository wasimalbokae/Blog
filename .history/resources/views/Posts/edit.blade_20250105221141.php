        ...
        <div class="card-body">
            <div class="row ">
                <div class="col-6">
                    ...
                    @foreach($post->tags->word as $tag)
                        @if(in_array($post->tags,  $tags))
                            <p>{{ $tag->word }} هو مختار</p>
                        @else
                            <p>{{ $tag->word }} هو غير مختار</p>
                        @endif
                    @endforeach

                    {{-- @foreach ($tags as $tag)
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input"  name="tags[]"  id="tags_{{$tag->word}}" value="{{$tag->id}}" checked>
                        <label class="form-check-label" for="tags_{{$tag->word}}">{{$tag->word}}</label>
                    </div>
                    @endforeach --}}
