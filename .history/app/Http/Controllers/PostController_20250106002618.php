...
class PostController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $currentTags = $post->tags->pluck('id')->toArray(); // الحصول على IDs الـ Tags الحالية
        $newTags = $request->tags; // الـ Tags الجديدة من الطلب

        // تحقق من وجود تغيير
        if ($currentTags != $newTags) {
            $post->tags()->sync($newTags); // مزامنة الـ Tags (حذف القديمة وإضافة الجديدة)
        }
