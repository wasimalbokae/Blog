...
class PostController extends Controller
{
    ...
    {
        $post = Post::with("tags")->find($id); // جلب الـ Post مع الـ Tags المرتبطة به
        $change = false;
        dd($request->tags);
        // تحقق من وجود تغيير في الـ Tags
        ...
        // إذا كان هناك تغيير، قم بحذف جميع الـ Tags الحالية وإضافة الـ Tags الجديدة
        if ($change)
        {
            $post->tags()->detach(); // إزالة جميع العلاقات بين الـ Post والـ Tags
            $post->tags()->attach($request->tags); // إضافة الـ Tags الجديدة
        }
            if($post && $post->id_user==Auth::user()->id)
            {
